...
class FM(nn.Module):
    def __init__(self,
        ...
        # dense特征二阶表示 每个field都做一次变换到 embed
        self.fm_2nd_order_dense_linear = nn.ModuleList([
            nn.Linear(1, emb_size)
            for _ in range(self.dense_feature_size)
        ])
        # 可选的另一种方式 dense特征二阶表示 直接变换到 emb_size
        # self.fm_2nd_order_dense_linear = nn.Linear(self.dense_feature_size, emb_size)
    ...
    def forward(self, X_sparse, X_dense=None):
        ...
        if X_dense is not None:
            # (bs, 1) (1, 8) -> (bs, 8)
            # print(self.fm_2nd_order_dense_linear[0](X_dense[:, 0].unsqueeze(1)).size())
            # 二阶dense linear后生成 torch.Size([bs, 8]) 中间扩充一维变成 torch.Size([bs, 1, 8]) 方便后面concat
            fm_2nd_order_dense = [
                linear(X_dense[:, i].unsqueeze(1)).unsqueeze(1)
                for i, linear in enumerate(self.fm_2nd_order_dense_linear)
            ]
            
            # 可选的另一种二阶dense方式 (bs, 13) (13, 8) -> (bs, 8) -> (bs, 1, 8)
            # fm_2nd_order_dense = self.fm_2nd_order_dense_linear(X_dense).unsqueeze(1)
            
            # batch_size, dense_feature_nums, emb_size  torch.Size([bs, 13, 8])
            fm_2nd_order_dense = torch.cat(fm_2nd_order_dense, dim=1)
        
            # torch.Size([bs, 39, 8])
            fm_2nd_order = torch.cat([fm_2nd_order_dense, fm_2nd_order_sparse], dim=1)
            
        else:
            fm_2nd_order = fm_2nd_order_sparse
            
        # 先求和再平方 batch_size, emb_size torch.Size([bs, 8])
        square_of_sum = torch.pow(torch.sum(fm_2nd_order, dim=1), 2)

        # 先平方再求和 batch_size, emb_size torch.Size([bs, 8])
        sum_of_square = torch.sum(torch.pow(fm_2nd_order, 2), dim=1)
