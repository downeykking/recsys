...
from model_dense_cross import FM
...
fix_seed(seed)
train_dataset, train_loader, valid_dataset, valid_loader, sparse_feature_columns, dense_feature_size = load_data(
    batch_size=batch_size)

model = FM(sparse_feature_columns, dense_feature_size).to(device)
...
def train(epoch):
    ...
    for x in train_loader:
        sparse_feat, dense_feat, labels = x[0].to(device), x[1].to(device), x[2].to(device)

        # Forward pass
        outputs = model(sparse_feat, dense_feat)
...
def valid(model):
    ...
    with torch.no_grad():
        ...
        for x in valid_loader:
            sparse_feat, dense_feat, labels = x[0].to(device), x[1].to(
